...
# SYSTEM FIX: If database file is missing on the cloud server, auto-create it instantly to bypass Streamlit validation freezes
if not os.path.exists(DB_FILE):
    conn_init = sqlite3.connect(DB_FILE)
    cursor = conn_init.cursor()
    # Create the core schema tables so the frontend queries never throw exceptions on boot
    cursor.execute('''CREATE TABLE IF NOT EXISTS system_config (key TEXT PRIMARY KEY, value TEXT)''')
    cursor.execute('''CREATE TABLE IF NOT EXISTS nea_telemetry (timestamp TEXT, hawker_centre TEXT, stall_id TEXT, zone_cluster TEXT, fill_level REAL, lid_breaches_count INTEGER, pir_wakeups_count INTEGER, rat_detections_count INTEGER, deterrence_triggered INTEGER, nea_division TEXT)''')
    # Inject default fallback system configs matching your presentation rules
    cursor.executemany("INSERT OR IGNORE INTO system_config VALUES (?, ?)", [("current_relay_limit", "15"), ("countermeasure_interval", "5")])
    conn_init.commit()
    conn_init.close()
